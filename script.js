...
const elements = {
    ...
    signalTimeTop: document.getElementById('signal-time-top'),
    // Элементы для флагов
...
function resetSignalUI() {
    ...
    elements.signalTimeTop.textContent  = '--:--:--';
}

function updateSignalArea(signalData, isRestore = false) {
    const { isBuy, startTime } = signalData;
    ...
    elements.signalTimeTop.textContent = formatTimeWithSeconds(startTime);
}
...
function startSignalProcess() {
    ...
    // Через заданную задержку завершаем процесс
    setTimeout(() => {
